...
Cairn 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class Cairn:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".cairn" / "runs")
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Cairn.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Cairn.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Cairn.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Cairn.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Cairn.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Cairn.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Cairn.retry_notice()
        if "<final>" in raw:
            final = Cairn.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Cairn.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Cairn.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Cairn.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Cairn.extract_raw(body, key)
