...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "cairn" / "agent" / "welcome" / "screen"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "cairn.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("cairn.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"CAIRN_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "cairn.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("cairn.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = cairn_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = cairn_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = cairn_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "CAIRN_PROVIDER=openai",
                "CAIRN_OPENAI_API_BASE=https://www.right.codes/codex/v1",
                "CAIRN_OPENAI_API_KEY=sk-project-openai",
                "CAIRN_OPENAI_MODEL=gpt-5.4",
                "CAIRN_DEEPSEEK_API_KEY=sk-project-deepseek",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "cairn.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = cairn_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        ...
        ), patch("cairn.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("cairn.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            cairn_pkg.build_agent(args)
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .cairn/.\n"
    ...
    index_path = tmp_path / ".cairn" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".cairn" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".cairn" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .cairn/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".cairn" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".cairn" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(cairn_pkg.__file__).as_posix().endswith("/cairn/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(cairn_pkg.main)
    assert callable(cairn_pkg.build_agent)
    assert callable(cairn_pkg.build_arg_parser)
